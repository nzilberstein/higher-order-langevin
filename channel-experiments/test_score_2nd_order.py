...
parser.add_argument('-d', '--discretization_scheme', type=str , default='BAOAB')
...
for meta_idx, (spacing, pilot_alpha) in tqdm(enumerate(meta_params)):
    ...
    for snr_idx, local_noise in tqdm(enumerate(noise_range)):
        ...
        for step_idx in tqdm(range(0, val_config.model.num_classes, args.spacing_classes)):
            ...
            if args.discretization_scheme == 'BAOAB':
                ...
                # Compute gradient
                meas_grad = torch.matmul(forward_h, torch.matmul(forward, current) - y) / (local_noise/2. + current_sigma ** 2)
            ...
            for inner_idx in range(val_config.sampling.steps_each):
                if args.discretization_scheme == 'BAOAB':
                    ...
                    vT = np.exp(- gamma * alpha) * vT + np.sqrt(MM * beta_noise * temp * (1 - np.exp(-2 * gamma * alpha))) * torch.randn_like(current)
...
if args.discretization_scheme == 'BAOAB':
    torch.save(save_dict, os.path.join(result_dir, 'results_lang_2ndorder_BAOAB.pt'))
elif 'ABO':
    torch.save(save_dict, os.path.join(result_dir, 'results_lang_2ndorder_ABO.pt'))